...
import 'package:login_screen/src/components/constants.dart';
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(
              width: 26), // Add some space between the line and the image
          Image.asset(
            'assets/images/sofrecom.png',
            height:
                65, // Adjust the height of the image as per your requirement
          ),
          const SizedBox(
              width: 20), // Add some space between the image and the line
          Flexible(
            child: Container(
              height: 10,
              color: kPrimaryColor,
...
class AuthService {
  ...
  static const String loginUrl =
      'http://10.0.2.2:8080/Humeur_salarie/api/login';
  ...
  Future<bool> login(BuildContext context, String matricule, String mdp) async {
    try {
      final response = await http.post(
        ...
        headers: {
          'Content-Type':
              'application/json', // Specify that you are sending JSON data
        },
        body: json.encode({
          // Use the json.encode method to convert the data to JSON format
          'matricule': matricule,
          'mdp': mdp,
        }),
      );
        ...
        if (loginSuccess) {
          ...
          if (role == 'manager') {
            ...
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                builder: (_) =>
                    HomePage(matricule: matricule), // Pass the matricule here
              ),
            );
          }
        }
